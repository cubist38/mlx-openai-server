...
Single-model-only operation and the lack of a registry were resolved after this
document was written; see [MODEL-LIFECYCLE.md](MODEL-LIFECYCLE.md).
...
- Single-model mode keeps the model in memory for the server lifetime; the
  multi-model registry loads on demand and unloads on the retention timer, see
...
- **mlx** (0.32): Apple Silicon array framework
- **mlx-lm** (0.31.3): Text-only language models
- **mlx-vlm** (0.6.17): Multimodal vision-language models
- **mlx-whisper** (0.4.3): Audio transcription
- **mlx-embeddings** (0.0.5): Text embeddings
- **mflux** (0.19.1, required): image generation and editing
...
- [x] Multi-model registry (load multiple models, route by model ID)
- [x] Model hot-swapping (load/unload without restart)
- [x] Model metadata API (capabilities, context length, etc.)
- [x] Model versioning and aliases
...
- [x] Model warmup API (preload without request) – `POST /v1/models/load`
