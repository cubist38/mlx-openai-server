...
class ErrorResponse(OpenAIBaseModel):
    ...
    code: str | int = Field(..., description="The error code (string or integer).")
...
class ChatCompletionRequestBase(OpenAIBaseModel):
    ...
    stop: str | list[str] | None = Field(
        None, description="Stop sequence(s) or list of stop sequences."
    )
...
class ImageGenerationRequest(OpenAIBaseModel):
    ...
    negative_prompt: str | None = Field(
        None,
        description="A text description of elements or concepts to avoid in the generated image(s). The maximum length is 1000 characters.",
        max_length=1000,
    )
    ...
    seed: int | None = Field(
        default=None,
        description="Seed for reproducible generation. If None, a random seed will be used.",
    )
...
class TranscriptionRequest(OpenAIBaseModel):
    ...
    presence_penalty: float | None = Field(
        default=None, description="The presence penalty for the transcription"
    )
