"""Tool parser for Llama4 pythonic tool calls."""
...
class Llama4PythonicToolParser(BaseToolParser):
    ...
    def parse(self, content: str) -> tuple[list[dict[str, Any]], str]:
        ...
        end_idx = content.rfind(cast("str", self.tool_close))
        ...
        try:
            ...
            if isinstance(parsed, ast.List) and all(isinstance(e, ast.Call) for e in parsed.elts):
                tool_calls = [self._handle_single_tool(cast("ast.Call", e)) for e in parsed.elts]
    ...
    def parse_stream(self, chunk: str | None = None) -> tuple[Any | None, bool]:
        ...
        if not self.parsing_tool:
            if self.tool_open in chunk:
                ...
                if cast("str", self.tool_close) in after_open:
                    close_idx = after_open.find(cast("str", self.tool_close))
                    self.parsing_tool = False
                    # Extract the complete tool call content
                    tool_content = (
                        self.tool_open + after_open[: close_idx + len(cast("str", self.tool_close))]
                    )
                    tools, remaining = self.parse(tool_content)
                    if tools:
                        # Return content before tool + content after tool (if any)
                        after_close = after_open[close_idx + len(cast("str", self.tool_close)) :]
        ...
        if cast("str", self.tool_close) in chunk:
            close_idx = chunk.find(cast("str", self.tool_close))
            # Try to parse the complete buffered content
            complete_tool = (
                self.tool_open
                + self.buffer[
                    : self.buffer.find(cast("str", self.tool_close))
                    + len(cast("str", self.tool_close))
                ]
            )
