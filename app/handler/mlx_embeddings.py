...
class MLXEmbeddingsHandler:
    ...
    def __init__(self, model_path: str, max_concurrency: int = 1) -> None:
        ...
        ----------
        model_path : str
            Path to the embeddings model to load.
    ...
    async def get_models(self) -> list[dict[str, Any]]:
        ...
        -------
        list[dict[str, Any]]
            List of model metadata dictionaries (keys: 'id', 'object', 'created', 'owned_by').
        """
        try:
            return [
                {
    ...
    async def initialize(self, _config: dict[str, Any]) -> None:
        ...
        _config : dict[str, Any]
            Dictionary containing queue configuration (not currently used).
    ...
    async def generate_embeddings_response(self, request: EmbeddingRequest) -> list[list[float]]:
        ...
        ------
        HTTPException
            If the request queue is full or generation fails.
        """
        try:
            ...
            raise HTTPException(status_code=HTTPStatus.TOO_MANY_REQUESTS, detail=content) from e
        except Exception as e:
    ...
    async def _process_request(self, request_data: dict[str, Any]) -> list[list[float]]:
        ...
        ------
        ValueError
            If the request type is unknown.
        """
        try:
