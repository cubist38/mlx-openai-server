...
class MLXLMHandler:
    ...
    async def generate_text_response(self, request: ChatCompletionRequest) -> str | dict[str, Any]:
        """
        ...
        Returns:
            str or dict: Complete response or structured data.
        """
        request_id = f"text-{uuid.uuid4()}"
    ...
    async def _process_request(self, request_data: dict[str, Any]) -> str | list[float]:
        """
        ...
        Returns:
            str or list[float]: The model's response or embeddings.
        """
        try:
    ...
    async def _prepare_text_request(
        ...
        try:
            ...
            # Normalize stop to list if it's a string
            if "stop" in request_dict and isinstance(request_dict["stop"], str):
                request_dict["stop"] = [request_dict["stop"]]
            # Generate random seed if None
            if request_dict.get("seed") is None:
                request_dict["seed"] = random.randint(0, 1_000_000)
            ...
            if request_dict.get("response_format", None):
                ...
                if response_format and response_format.get("type") == "json_schema":
                    ...
                    if json_schema:
                        request_dict["schema"] = json_schema.get("schema", None)
