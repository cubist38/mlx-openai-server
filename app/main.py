...
def parse_args():
    ...
    parser.add_argument("--pipeline", action="store_true", help="Use pipeline mode for the model. Only works with language models (`lm` model type).")
        ...
        try:
            ...
            if config_args.model_type == "multimodal":
                ...
                handler = MLXLMHandler(
                    ...
                    tool_call_parser=getattr(config_args, 'tool_call_parser', None),
                    reasoning_parser=getattr(config_args, 'reasoning_parser', None),
                    pipeline=getattr(config_args, 'pipeline', False)
                )       
            # Initialize queue
