    ...
    def __call__(self, prompt: str, seed: int = 42, **kwargs: Any) -> Image.Image:
        ...
        try:
            ...
            raise ModelGenerationError(
                f"Failed to prepare configuration. {type(e).__name__}: {e}"
            ) from e
...
class FluxStandardModel(BaseFluxModel):
    ...
    def _generate_image(self, prompt: str, seed: int, config: Config) -> Image.Image:
        ...
        try:
            ...
            raise ModelGenerationError(
                f"Standard model generation failed. {type(e).__name__}: {e}"
            ) from e
        else:
            return result.image
...
class FluxKontextModel(BaseFluxModel):
    ...
    def _generate_image(self, prompt: str, seed: int, config: Config) -> Image.Image:
        ...
        try:
            ...
            raise ModelGenerationError(
                f"Kontext model generation failed. {type(e).__name__}: {e}"
            ) from e
        else:
            return result.image
...
class FluxModel:
    ...
    def __init__(
        ...
        try:
            ...
            error_msg = f"Failed to initialize FluxModel.{type(e).__name__}: {e}"
            logger.error(error_msg)
            raise ModelLoadError(error_msg) from e
